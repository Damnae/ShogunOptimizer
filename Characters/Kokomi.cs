namespace ShogunOptimizer.Characters
{
    public class Kokomi : Character
    {
        ...
        public override double Calculate(string property, Build build, HitType hitType, Enemy enemy)
        {
            switch (property)
            {
                case PropertyAttack1: return CalculateDamage(build, (0.6838 * GetTalentAttackScaling(AttackLevel)) * GetAtk(build), DamageType.Normal, Element.Hydro, hitType, enemy);
                case PropertyAttack2: return CalculateDamage(build, (0.6154 * GetTalentAttackScaling(AttackLevel)) * GetAtk(build), DamageType.Normal, Element.Hydro, hitType, enemy);
                case PropertyAttack3: return CalculateDamage(build, (0.9431 * GetTalentAttackScaling(AttackLevel)) * GetAtk(build), DamageType.Normal, Element.Hydro, hitType, enemy);
                case PropertyCharged: return CalculateDamage(build, (1.4832 * GetTalentAttackScaling(AttackLevel)) * GetAtk(build), DamageType.Charged, Element.Hydro, hitType, enemy);

                case PropertySkillDamage: return CalculateDamage(build, (1.0919 * GetTalentPercentageScaling(SkillLevel)) * GetAtk(build), DamageType.Skill, Element.Hydro, hitType, enemy);
                ...
                case PropertyBurstInitial: return CalculateDamage(build, (0.1042 * GetTalentPercentageScaling(BurstLevel)) * GetMaxHp(build), DamageType.Burst, Element.Hydro, hitType, enemy);
                ...
                case PropertyBurstAttack3C1: return CalculateDamage(build, 0.3 * GetMaxHp(build), DamageType.None, Element.Hydro, hitType, enemy);
